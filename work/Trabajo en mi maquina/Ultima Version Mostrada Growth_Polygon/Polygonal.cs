	...
	public class Simple_Polygonal: Polygonal
	{
		...
		virtual protected bool Get_Polygonal(int dist, out Polygonal R,out ArrayList Reversed, bool V_Int)
		{
				...
				while( i < R.CantOfVertex()-1 )
				{
					...
					{					
						...
						else
						{
							...
							PAux[0] = R[i - 1].Clone();
							PAux[1] = R[ i ].Clone();
							PAux[2] = this[LinkToOriginal[i-1][ 0 ]].Clone();
							PAux[3] = this[LinkToOriginal[i-1][ 1 ]].Clone();
			...
			if(list.Count != 0)
			{
				...
				{
					del = true;
					this.Remove_Vertex( (int)list[0] );
				}
				else
				{				
					if( Math.Abs( (int)list[list.Count-1]  - (int)list[list.Count-2] ) != 1 )
					{
						del = true;
						this.Remove_Vertex( (int)list[list.Count-1] );
					}

					int tag  = (int)list[list.Count-1];

					for(int k = list.Count-2; k>0; k--)
					{
						if( ( Math.Abs(      tag     - (int)list[  k  ] ) != 1 ) && 
							( Math.Abs( (int)list[k] - (int)list[k - 1] ) != 1 ) )
						{
							del = true;
							this.Remove_Vertex( (int)list[k] );
						}
						tag = (int)list[  k  ];
					}

					if( Math.Abs( (int)list[0]  - (int)list[1] ) != 1 ) 
					{
						del = true;
						this.Remove_Vertex( (int)list[0] );
					}
			...
			for(i=0; i<R.CantOfVertex()-2; i++)
			{
				...
				SB1 = Segment.Proyect(this[LinkToOriginal[(i+R.CantOfVertex())%R.CantOfVertex()][1]],S1,S2);
				SE1 = S2;

				SB2 = S2;
				SE2 = Segment.Proyect(this[LinkToOriginal[(i+1)%R.CantOfVertex()][0]],S2,S3);

				R_Point P1,P2;				

				for(int k=0;k<R.CantOfVertex();k++)
				{
					P1 = this[LinkToOriginal[k][0]];					
					P2 = this[LinkToOriginal[k][1]];		
			...
			for(i=list.Count-3;i>=0;i-=3)
			{
				...
				Reversed.Insert((int)list[i],false);
			}
			return true;
		}
		virtual public ArrayList Parallel(int dist, out Polygonal Result)
		{
			...
			do
			{
				...
				try
				{
					if ( !Get_Polygonal( dist, out Result,out Reversed , (i == Count) ) )throw new Exception("Lanzo una excepcion");//Ojo
					finish = false;
				}
				catch( InvertPeak  )
				{
			...
			ArrayList aux = P.GetValid(-1, Reversed);
			ArrayList Faces = new ArrayList();
		...
		override protected bool Get_Polygonal(int dist, out Polygonal R, out ArrayList Reversed, bool V_Int)
		{
			...
			if(list.Count != 0)
			{
				...
				else
				{
					...
					if( ( (int)list[0] != 0 ) || ((int)list[list.Count-1] != this.CantOfVertex()-1) )
					{
						distinct = true;
						del = true;
						this.Remove_Vertex( (int)list[list.Count-1] );
