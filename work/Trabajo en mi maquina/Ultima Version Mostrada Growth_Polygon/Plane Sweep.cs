	...
	public class PlaneSweep
	{
		...
		private void HandleEvent( Event e, out ArrayList Seg_Close, out ArrayList Seg_Open, out Segment[] Extremes )
		{
			...
			if( S_Close == null )
			{
				...
				if( node1 != null )
				{					
					// Ojo Ver si entra aqui o es redundante

					/* Si no cierra nadie puede ser que existan segmentos que corta y debo buscar en la estructura 
					 * de la sweep line y agregarlo al sclos para que el resto del analisis funcione igual*/

					S_Close = node1.Value() as Segment;
					if( ( S_Close.GetLowerEnd().x != e.GetPoint().x ) ||
						( S_Close.GetLowerEnd().y != e.GetPoint().y ) )
					{
						/*Si el segmento era uno que cortaba a la sweep line en el punto debo
					  *  agregarlo a la lista de los que abren*/
			...
			if( S_Close != null )
			{
				...
				do 
				{
					...
					if( node2 != null) 
					{
						...
						{
							/* Si el segmento cortaba debo agregarlo a la lista de ls que abren para ser insertado*/
							/* Si el segmento de S_Close era uno que cortaba ya lo agregue a la lista y no lo trato aqui*/
							if( Seg_Open == null ) Seg_Open = new ArrayList();
			...
			if( ( Seg_Open != null ) && ( Seg_Open.Count > 0 ) )
			{
				...
				{
					/*Este ciclo lo hago para sacar los nuevos segmentos de forma ordenada 
					 * para poder sacar las regiones que comienzan sin tener que ordenar los 
					 * segmentos que abren */
				...
				/* Estos dos if se hacen para determinar los posibles nuevos eventos que surgen entre
				 * los extremos y el primer y ultimo segmento de los que abren*/
				if( init ) 
				{
			...
			{
				/* En el evento que trate nadie abrio por lo que tengo que ver 
				 * si los deos extremos se cortan formando un nuevo evento*/
