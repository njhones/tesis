...
namespace Growth_Polygon
{
		...
		private void HandleEvent( Event e, out ArrayList Seg_Close, out ArrayList Seg_Open, out Segment[] Extremes )
		{
			...
			if( S_Close != null )
			{
				...
				do 
				{
					...
					if( node2 != null) 
					{
						( (Segment)node2.Value() ).flag = false;
						Aux.Insert(0,node2);
	...
	/// 
	/// </summary>
	public class IntersectionTree : RBTree  
	{
		...
		override public bool Insert( Object elem, out Object Result)
		{
			...
			if( ( y != Nil ) && ( Compare( y.Value(), elem ) == 0 ) ) 
			{
				...
				if( ( (Intersection)Result ).GetMajor() < ( (Intersection)elem ).GetMajor() )
				{
					y.setValue( elem );
					exist = true;
				}
			...
			if( !exist )
			{
				z = new RBNode(Nil,Nil,Nil,Red,elem);
				z.setParent( y );
			...
			x = Antecessor( z );
			while( ( x != null ) &&
				   ( ( (Intersection)x.Value() ).GetMinor() >= ( (Intersection)z.Value() ).GetMinor() ) &&
				   ( ( (Intersection)x.Value() ).GetMajor() <= ( (Intersection)z.Value() ).GetMajor() ) )
			{
				y = Antecessor( x );
				Delete( x );
				x = y;
		...
		public IntersectionTree():base() 
		{
			Count = 0;
		}
		override protected int Compare(Object E1, Object E2 )
		{
			Intersection Id1 = ( Intersection )E1;
			Intersection Id2 = ( Intersection )E2;

			if( Id1.GetMinor() == Id2.GetMinor() ) 
				return 0;
		...
		public bool Contain( int id, out RBNode I )
		{
			I = null;
			RBNode aux = Root;
			while( aux != Nil )
			{
				if( ( id >= ((Intersection)aux.Value()).GetMinor() ) && 
					( id <= ((Intersection)aux.Value()).GetMajor() ) ) 
				{
					I = aux;
					return true;
				}
		...
		public RBNode GetIntersection( int id )
		{
			RBNode minor = null;
			RBNode aux = Root;
			while( aux != Nil )
			{
				if( id == ((Intersection)aux.Value()).GetMinor() )  return aux;
				...
				else 	aux = aux.LeftChild();				
			}

			return minor;

		}
		public RBNode GetMinor()
		{
			if( Root == Nil ) return null;
			
			RBNode Aux = Root;
			while( Aux.LeftChild() != Nil ) Aux = Aux.LeftChild();
	...
	public class Intersection
	{
		...
		public Intersection( R_Point pt, int minor, int major )
		{
			Pt = pt;
			id_Minor = minor;
			id_Major = major;
	...
	public class IntervalNode
	{
		...
		public VertexOfRegion Val;

		public IntervalNode Right;
		public IntervalNode Left;

		public IntervalNode(R_Point pt, IntervalNode l, IntervalNode r)
		{
			Val.Pt = pt;
			Left = l;
			Right = r;
		...
		public IntervalNode(R_Point pt, IntervalNode l, IntervalNode r, Segment s1, Segment s2)
		{
			Val.Pt = pt;
			Left = l;
			Right = r;
	...
	public class Interval
	{
		...
		public void SetLeft( IntervalNode L )
		{
			Left = L;			
		}
		public void SetRight( IntervalNode R )
		{
			Right = R;
		}
		...
		{
			Right.Right = new IntervalNode( pt, Right, null );
			Right = Right.Right;	
		}
		public void AddLeft( R_Point pt )
		{
			Left.Left = new IntervalNode( pt, null, Left );
			Left = Left.Left;			
		}
		#endregion
		
		public Interval(Region R,int f,int id, bool exterior)
		{
			I_Left = null;
			I_Right = null;
			Parents = null;
		...
		public Interval(Interval[] P,Region R,int f,int id, bool exterior)
		{
			I_Left = null;
			I_Right = null;
			Parents = P;
	...
	public class Region
	{
		...
		public ArrayList GetVertex()
		{	
			return I.GetVertexes();
		}	
		...
		public Region Clone()
		{
			...
			Result.SetInterval( I );
			Result.flag = flag;
			if( Referenced ) Result.SetReferenced();
