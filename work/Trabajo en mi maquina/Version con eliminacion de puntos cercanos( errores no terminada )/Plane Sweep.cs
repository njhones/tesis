...
{	
	...
	public abstract class Heap  
	{
		...
		public void Arrange()
		{
			...
			if( n % 2 == 0 )
			{
				if( Compare( List[n-1],List[(n/2)-1] ) == -1 )
				{
					Aux = List[n-1];
					List[n-1] = List[(n/2)-1];
					List[(n/2)-1] = Aux;
			...
			{
				bool first;
				if( Compare( List[(i/2)-1],Min(List[i-1],List[i],first)) == 1 )
				{
					int pos = i;
					if( first ) pos--;
					...
					do
					{
						...
						if( ( pos+1<Count ) && ( Compare( List[Parent],Min(List[Pos+1],List[Pos],first)) == 1 ) )
						{
							change = true;
							if( first ) pos++;							
						}
						...
						{
							if( ( pos<Count ) && ( Compare( List[Parent],List[Pos]) == 1 ) )
							{									
								change = true;
							}							
						}
		...
		public void Insert(Object Elem)
		{
			...
			while( ( pos > 0 ) && ( change ) )
			{
				...
				if( Compare( List[ Parent ], List[pos] ) == 1)
				{
					Aux = List[ Parent ];
					List[ Parent ] = List[ pos ];
					List[ pos ] = Aux;
		...
		protected abstract int Compare( Object E1, Object E2 );
		protected Object Min( Object E1, Object E2, out bool first )
		{
			if( Compare( E1,E2 ) == 1 ) 
			{
				first = false;
	...
	{
		private ArrayList Medians;
		
		public SegmentTree( ArrayList list )
		{
			for(int i=0; i<list.Count;i++)
			{
				Medians.Add( ( ( R_Point )list[i] ).x );
				Medians.Add( ( ( R_Point )list[i] ).y );
			}
			Medians.Sort();
		}
		public void Add( Segment S )
		{
			STSegment stSeg = new STSegment( S );
	...
	{
		protected R_Point LinealSeg;
		protected Segment Seg;
		
		public STSegment( Segment S )
		{
            Seg = S;
			LinealSeg = new R_Point();
			if( S.GetInit().x < S.GetEnd().x )
			{
				LinealSeg.x = S.GetInit().x;
				LinealSeg.y = S.GetEnd( ).x;
			}
			else
			{
				LinealSeg.y = S.GetInit().x;
				LinealSeg.x = S.GetEnd( ).x;
			}
		}
	}
	/// <summary>
	...
	public class STNode
	{
		...
		protected ArrayList News;
		public int GetDistOfSon()
		{
			return dist;
		}
		...
		protected void SortByLeft( ArrayList List )
		{
		}
		protected void SortByRight( ArrayList List )
		{
		}
		public STNode()
		{
	...
	public class STTree
	{
		...
		private void SortMedians()
		{
		}
		public STTree(ArrayList Segments)
		{
			for (int i=0; i< Segments.Count; i++)
			{
				Medians.Add(((Segment)Segments[i]).GetInit().x);
				Medians.Add(((Segment)Segments[i]).GetEnd().x);
			}
			SortMedians();
			Nodes = new ArrayList(Medians.Count);
		}
	}
	/// <summary>
	/// 
	...
	public class Event
	{
		...
		public void Add_S_Open(ArrayList S)
		{
			...
			{
				int cant = S.Count;
				for(int i =0; i<cant;i++) S_Open.Add((Segment)S[i]);
			}
		}
